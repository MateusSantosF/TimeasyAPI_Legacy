    ...
    public static class MockGenerator
    {
        ...
        static List<Teacher> GetTeachers(List<Subject> subjects, int count, Random random)
        {
            ...
            for (int i = 1; i <= count; i++)
            {
                ...
                // Defina a quantidade aleatória de disciplinas para o professor.
                int numPreferredSubjects = random.Next(1, 6); // Entre 1 e 5 disciplinas.

                for (int j = 0; j < numPreferredSubjects; j++)
                {
                    // Escolha aleatoriamente uma disciplina da lista de disciplinas.
                    int randomIndex = random.Next(subjects.Count);
                    Subject randomSubject = subjects[randomIndex];

                    // Adicione a disciplina às disciplinas preferidas do professor.
        ...
        static List<Interval> GenerateRandomInterval(Random random)
        {
            ...
            int numIntervals = random.Next(1, 4); // Entre 1 e 3 intervalos.

            for (int i = 0; i < numIntervals; i++)
            {
                int startHour = random.Next(8, 18); // Horário de início entre 8 e 17.
                int endHour = random.Next(startHour + 1, 19); // Horário de término após o início.
        ...
        static List<Subject> GetSubjects(List<Course> courses, int count, Random random)
        {
            ...
            foreach (Course course in courses)
            {
                for (int i = 1; i <= count; i++)
                {
                    ...
                    {
                        Id = Guid.NewGuid(),
                        CourseId = course.Id, // Associe a disciplina ao curso correspondente.
                    ...
                    // Adicione a disciplina à lista de disciplinas do curso.
